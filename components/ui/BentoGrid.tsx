...
export const BentoGridItem = ({
  ...
  const handleCopy = () =>{
    navigator.clipboard.writeText('jackson95_llp@live.com');
        ...
        <div className="w-full h-full absolute">
          {img && (
            <img
              src={img}
              alt={img}
              className={cn(imgClassName, "object-over, object-center")}
            />
          )}
        </div>
            ...
            <div className="flex gap-1 lg:gap-5 w-fit absolute -right-3 lg:right-2">
              <div className="flex flex-col gap-2 lg:gap-7 ">
                {["React.js", "Next.js", "Typescript", "Tailwind"].map((item) => (
                  <span
                    key={item}
                    className="py-2 lg:py4 px-3 text-xs lg:text-base opacity-50 lg:opacity-100 rounded-lg text-center bg-[#10132E] "
                  >
                    {item}
                  </span>
                ))}
              ...
              <div className="flex flex-col gap-2 lg:gap-7 ">
                {["MongoDB", "Node.js", "Express", "Git"].map((item) => (
                  <span
                    key={item}
                    className="py-2 lg:py4 px-3 text-xs lg:text-base opacity-50 lg:opacity-100 rounded-lg text-center bg-[#10132E] "
                  >
